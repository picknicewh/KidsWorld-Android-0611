...
.controller('Search_InfCtrl', function($scope, $state, $ionicPopup,$ionicScrollDelegate,WebService,CommonService) {

    //视图渲染前，从服务器取得图片数据
    $scope.$on('$ionicView.beforeEnter', function() {
        //隐藏头部导航栏
        if (platFlag == 1) {
            window.location.href = 'tooc://changenavigationtitle:?null&搜索资讯*null';
        } else if (platFlag == 0) {
            change_tb.setToolBar("search_con");
        }
    ...
    $scope.getSearchHis=function(){
        ...
        //});
        $scope.items_searchHis=CommonService.searchHisService.getInfHis();
    }
    //清空按钮
    ...
    $scope.search=function(){
        ...
        CommonService.searchHisService.addInfHis(key);
    ...
    $scope.searchByKey=function(){
        var res = WebService.getResourceListByName($scope.keyForSearch,2,6,$scope.pageNumber);
